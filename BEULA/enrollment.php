...
<section>
    ...
    <form method="post">
        ...
        <label for="eid">enrollmentid:</label>
        <input type="number" id="Pid" name="cid"><br><br>
        ...
        <input type="text" id="gr" name="gr" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO enrollment(enrollmentid,courseid,userid,enrollmentdate,grade) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $eid, $cid, $uid, $edt,$gr);
    ...
    <table border="5">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $eid = $row['enrollmentid']; // Fetch the enrollmentid
                echo "<tr>
                    ...
                    <td>" . $row['grade'] . "</td>
                    <td><a style='padding:4px' href='delete_enrollment.php?enrollmentid=$eid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_enrollment.php?enrollmentid=$eid'>Update</a></td> 
                </tr>";
            }
        } else {
