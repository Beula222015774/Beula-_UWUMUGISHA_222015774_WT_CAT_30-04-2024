...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO user(userid, username, email, gender) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $uid, $un, $e, $g);
...
<body>
    <center><h2>Table of user</h2></center>
    <table border="5">
        <tr>
            <th>userid</th>
            <th>username</th>
        ...
        <?php
        include('database_connection.php');
        // Prepare SQL query to retrieve all user
        $sql = "SELECT * FROM user";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td>" . $row['email'] . "</td>
                    <td>" . $row['gender'] . "</td>
                    <td><a style='padding:4px' href='delete_user.php?userid=$uid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_user.php?userid=$uid'>Update</a></td> 
                </tr>";
            }
        } else {
